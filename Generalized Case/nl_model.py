...
def nlmodel(x, params, inputs):
    ...
    GenVQ08 = (+gen08[9] * np.cos(delta08) - gen08[10] * np.sin(delta08))
    GenVD08 = (+gen08[9] * np.sin(delta08) + gen08[10] * np.cos(delta08))


    GenV = [bus00[0], bus00[1],GenVQ01[0],GenVD01[0],GenVQ02[0],GenVD02[0],GenVQ03[0],GenVD03[0],GenVQ04[0],GenVD04[0],GenVQ05[0],GenVD05[0],GenVQ06[0],GenVD06[0],GenVQ07[0],GenVD07[0],GenVQ08[0],GenVD08[0]]
    ...
    solu08 = unit1.unitmodel(gen08, pgen08, inputs08, PU_Scaler * LVline[16], PU_Scaler * LVline[17], delta08[0])
    #======Function for reference angle generation

    #----------state equations for calculating reference angle states------
    delta = np.empty((8))             # array parameter is just Wrated, in most cases 376.99 nothing to do with other parameters

    delta[0] = Wrated * (wconv01 - wgrid)
    delta[1] = Wrated * (wconv02 - wgrid)
    delta[2] = Wrated * (wconv03 - wgrid)
    delta[3] = Wrated * (wconv04 - wgrid)
    delta[4] = Wrated * (wconv05 - wgrid)
    delta[5] = Wrated * (wconv06 - wgrid)
    delta[6] = Wrated * (wconv07 - wgrid)
    delta[7] = Wrated * (wconv08 - wgrid)
